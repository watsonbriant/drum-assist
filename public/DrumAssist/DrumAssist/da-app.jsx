...
(function () {
  ...
  const S = window.DAStore;
  const A = window.DAAudio;
  const LANE_COLORS = window.DA_LANE_COLORS;
  const KICK_COLOR = window.DA_KICK_COLOR;
  ...
  function App() {
    const ui0 = S.loadUI();
    const [chart, setChart] = useState(function () {
      S.migrateLegacy();
    ...
    const [charts, setCharts] = useState(S.listCharts);
    ...
    useEffect(function () {
      let dead = false;
      (async function () {
        const rec = await S.loadAudio(chartRef.current.id);
        if (dead || !rec) return;
        try {
    ...
    return React.createElement("div", { className: "app", "data-mode": mode },
      ...
      React.createElement("div", { className: "body" },
        React.createElement("div", { className: "stage" },
          React.createElement(window.HighwayCanvas, {
        ...
        // side panel
        React.createElement(window.SidePanel, {
