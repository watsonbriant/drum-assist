...
(function () {
  ...
  // Lane palette (fixed gameplay colors)
  const LANE_COLORS = ["#e2483b", "#e8c020", "#2f86ea", "#33ad52"]; // red, yellow, blue, green
  const KICK_COLOR = "#ef8a17";
  const GREY = "#3a3b41";
  ...
  function HighwayCanvas(props) {
    ...
    function draw() {
      ...
      // beat grid lines
      const offset = P.chart.offset, bpb = window.DAStore.tsNum(P.chart);
      const spb = window.DAStore.secPerBeat(P.chart);
      ...
      for (let k = k0; ; k++) {
        ...
        const onBar = onBeat && window.DAStore.isBarStart(P.chart, beatIndex);
        ctx.beginPath();
    ...
    function onPointerDown(e) {
      ...
      let t = sc.t;
      if (t < 0) return;
      if (P.snapEnabled) t = snapTime(t, window.DAStore.secPerBeat(P.chart), P.chart.offset, P.snapDiv);
      if (t < 0) t = 0;
  ...
  window.DA_LANE_COLORS = LANE_COLORS;
  window.DA_KICK_COLOR = KICK_COLOR;
})();
